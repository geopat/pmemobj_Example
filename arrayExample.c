...
/*Here it does not work. WHY????????????????????????????????? ZALLOC DOESNT WORK*/

        TOID(struct array) expected = TOID_NULL(struct array);

        printf("ROOT POINTER IS %p\n",D_RW(*root_ptr));

        if (TOID_IS_NULL(*root_ptr)) { // If array is NULL, initialize it and then place the first element
            ...
            pmemobj_persist(pop, &D_RW(new_array)->capacity, sizeof(D_RW(new_array)->capacity));
            ...
            POBJ_ZALLOC(pop, &D_RW(new_array)->data, struct data, D_RW(new_array)->capacity * sizeof(struct data));
           ...
           // Assign the new array to the root pointer and persist the root pointer
            if(!__atomic_compare_exchange(root_ptr, &expected, &new_array, 0, __ATOMIC_RELAXED, __ATOMIC_RELAXED)){
                printf("Failed\n");
                POBJ_FREE(&new_array);
            }
...
int main(int argc, char *argv[]) {
	...
	pop = pmemobj_create(POOL_PATH, POBJ_LAYOUT_NAME(array), PMEMOBJ_MIN_POOL, 0666);
	int  k = 0 ; 
	if (pop == NULL) {
